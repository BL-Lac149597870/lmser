...
class ConvAutoEncoderManual(nn.Module):
    def __init__(self):
        ...
        self.weight1 = nn.Parameter(torch.Tensor(16, 1, 3, 3))
        ...
        self.weight10 = nn.Parameter(torch.Tensor(16, 1, 3, 3))
        self.bias10 = nn.Parameter(torch.Tensor(1))
...
if __name__ == '__main__':
    ...
    plot(displayImages, (4, 4), 16, './output/original.png')
    ...
    for epoch in range(EPOCH):
        ...
        plot(decoded, (4, 4), 16, './output/Epoch_' + str(epoch + 1) + '.png')
        scheduler.step()
...
displayImages = displayImages.reshape(-1, 1, 28, 28)
for i in range(displayImages.shape[0]):
    for m in range(14, 28):
        for n in range(28):
            displayImages[i, 0, m, n] = 0
plot(displayImages, (4, 4), 16, './output/masked.png')
displayImages = torch.tensor(displayImages)
displayImages = Variable(displayImages.cuda())
out, decoded = model(displayImages)
decoded = decoded.cpu().data.numpy()
decoded = decoded.reshape(-1, 1, 28, 28)
plot(decoded, (4, 4), 16, 'recovered.png')
encoded = []
encodedLabel = []
for step, (images, labels) in enumerate(trainLoader):
    inputs = Variable(images.cuda())
...
visualizeLabel = []
for i in range(10):
    digitIndex = np.array([j for j in range(60000) if encodedLabel[j] == i])
