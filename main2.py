...
LastEditTime: 2020-12-09 01:53:46
LastEditors: QHGG
Description: 
FilePath: /junc_tree/main2.py
'''
import torch
...
if __name__ == '__main__':
    ...
    plt.savefig('./CIFAR10/reconLoss.png')
    plt.close()
    ...
    plt.savefig('./CIFAR10/perdLoss.png')
    plt.close()
    plt.plot(x, testAcc)
    plt.title('Test Accuracy')
    plt.savefig('./CIFAR10/testAcc.png')
    plt.close()
    ...
    decoded = decoded.cpu().data.numpy()
    decoded = decoded.reshape(-1, 3, 32, 32)
    plot(decoded, (4, 4), 16, './CIFAR10/recovered.png')
    encoded = []
    encodedLabel = []
    for step, (images, labels) in enumerate(trainLoader):
        inputs = Variable(images.cuda())
    ...
    plt.savefig('./CIFAR10/visualize.png')
    plt.close()
